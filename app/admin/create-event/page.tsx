...
export default function CreateEventPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (status === "upcoming") {
      ...
      newEvent.gallery = galleryUrls.split(",").map(url => url.trim()).filter(Boolean);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-zinc-900/50 p-8 rounded-2xl border border-zinc-800">
          ...
          <div>
            ...
            <input type="url" id="image" name="image" required placeholder="https://images.unsplash.com/..." className="w-full bg-black border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 transition-colors" />
          </div>
